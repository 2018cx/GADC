...
from feature import get_features # -> torch.Tensor [N, D=1024]

# ---------------------------
# GoogLeNet probs / CE / entropy
# ---------------------------
@torch.no_grad()
def get_googlenet_probs_and_scores(dataloader, device: str):
    """
    Use pretrained GoogLeNet as classifier to produce 1000-d probabilities.
    Returns:
        probs_all:  [N, 1000]
        labels_all: [N]
    ...
    cls_model = googlenet(weights=weights, aux_logits=True).to(device).eval()
...
# ---------------------------
# Local swap refinement (optional)
# ---------------------------
...
def worker_run(
    ...
    for class_id in class_ids:
        ...
        probs_all, labels_all, ce_all, ent_all, pmax_all = get_googlenet_probs_and_scores(loader, device)
