...
@torch.no_grad()
def get_confidence_probs_and_scores(dataloader, device: str):
    weights = Inception_V3_Weights.DEFAULT
    inc = inception_v3(weights=weights, aux_logits=True, transform_input=False).to(device).eval()
...
def partial_single_sided_ot_cost_batched(
    ...
    epsilon: float = 10.0,
    n_iter: int = 100,
    capacity_scale: float = 1.2,    # >= 1.0; real-side per-column capacity multiplier
    ...
    assert capacity_scale >= 1.0, "capacity_scale must be >= 1 to make y-side inequality meaningful."
    ...
    if uniform_marginals or (mu is None):
        mu = torch.full((B, m), 1.0 / m, device=device, dtype=dtype)     # sum = 1
    else:
        mu = mu.to(device=device, dtype=dtype)

    if uniform_marginals or (nu_cap is None):
        # each real y gets capacity_scale/n; total = capacity_scale
    ...
    s = (nu_cap.sum(dim=1, keepdim=True) - 1.0).clamp_min(0.0)           # [B,1] dummy supply
    mu_aug = torch.cat([mu, s], dim=1)                                   # [B, m+1]
    nu = nu_cap                                                          # [B, n]

    # --- balanced Sinkhorn on augmented problem ---
    u = torch.ones((B, m + 1), device=device, dtype=dtype)
    v = torch.ones((B, n), device=device, dtype=dtype)

    for _ in range(n_iter):
        Kv = torch.bmm(K, v.unsqueeze(-1)).squeeze(-1).clamp_min(EPS)   # [B,m+1]
        u = mu_aug / Kv
        KTu = torch.bmm(K.transpose(1, 2), u.unsqueeze(-1)).squeeze(-1).clamp_min(EPS) # [B,n]
        v = nu / KTu

    T_aug = u.unsqueeze(2) * K * v.unsqueeze(1)                          # [B, m+1, n]
    T_real = T_aug[:, :m, :]                                             # drop dummy row
    cost = torch.sum(T_real * C_real, dim=(1, 2))                        # only real transport term
    return cost
...
# ---------------------------
# Cost for candidate sets = OT + λ * moment_diag + α * KL(soft-target) + (可选 CE/Entropy)
# ---------------------------
def compute_costs_in_batches(
    ...
    sinkhorn_iter: int,
    batch_size: int,
    capacity_scale: float,              # y-side capacity multiplier
    # moment term
    moment_weight: float,
    ...
    for start in range(0, M, batch_size):
        ...
        if use_moment:
            ...
            moment_diag = diag_to_target_batched(mu_b, var_b, mu_t, var_t)
...
def global_refine_with_chunks(
    ...
    def objective_for_sel(sel_idx: torch.Tensor) -> float:
        ...
        if (mu_t is not None) and (var_t is not None) and (moment_weight > 0.0):
            ...
            moment_diag = diag_to_target_batched(mu_b, var_b, mu_t, var_t)[0]
            if scale_match is not None:
                moment_diag = moment_diag * scale_match
            total = total + moment_weight * moment_diag
    ...
    for _ in tqdm(range(1, n_select), desc="Greedy Sinkhorn Select"):
        ...
        for start in range(0, B, batch_size):
            ...
            if (mu_t is not None) and (var_t is not None) and (moment_weight > 0.0):
                ...
                moment_diag = diag_to_target_batched(mu_b, var_b, mu_t, var_t)  # [b]
                if scale_match is not None:
                    moment_diag = moment_diag * scale_match
                total = total + moment_weight * moment_diag
...
def worker_run(
    ...
    for class_id in class_ids:
        ...
        probs_all, labels_all, ce_all, ent_all, pmax_all = get_confidence_probs_and_scores(loader, device)
        ...
        # === 现场计算 mu_t, var_t（不再从 stats_dir 读） ===
        mu_t, var_t = (None, None)
        if moment_weight > 0.0:
            # 用 float32 计算更稳定，即使 feats 是 fp16
            feats_full_fp32 = feats.float()
            mu_t = feats_full_fp32.mean(dim=0)
            var_t = feats_full_fp32.var(dim=0, unbiased=True) if feats_full_fp32.size(0) > 1 else torch.zeros_like(mu_t)
