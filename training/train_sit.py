...
import pandas as pd
import random
from dataset.dataset_ddc import CustomDataset_REPA as CustomDataset
...
def main(args):
    ...
    ema_2 = deepcopy(model).to(device)
    requires_grad(ema, False)
    requires_grad(ema_2, False)
    ...
    if args.resume_ckpt is not None:
        if os.path.isdir(args.resume_ckpt):
            ...
            ema_2.load_state_dict(checkpoint["ema_2"])
    ...
    if args.resume_ckpt is None:
        ...
        update_ema(ema_2, model.module, decay=0)
    ...
    grad_accum = 1
    assert isinstance(grad_accum, int) and grad_accum >= 1, "args.grad_accum must be an integer >= 1"

    logger.info(f"Training for {args.epochs} epochs... (grad_accum={grad_accum})")

    log_steps = 0
    running_loss = 0.0
    running_denoising_loss = 0.0
    ...
    for epoch in range(args.epochs):
        ...
        for x, class_idx, zs in loader:
            ...
            with torch.no_grad():
                x_latent = vae.encode(x.to(device)).latent_dist.sample().mul_(0.18215)
            ...
            if accum_counter % grad_accum == 0:
                ...
                update_ema(ema_2, model.module, decay=0.999945)
                ...
                with torch.no_grad():
                    if rank == 0 and train_steps % 5000 == 0:
                        samples_dir = f"{args.results_dir}/samples"
                        if not os.path.exists(samples_dir):
                            os.makedirs(samples_dir)

                        pred_img = vae.decode(predicted_x / 0.18215).sample
                        pred_img = torch.clamp(127.5 * pred_img + 128.0, 0, 255)\
                                    .permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
                        for i, sample in enumerate(pred_img):
                            Image.fromarray(sample).save(
                                f"{samples_dir}/{train_steps:07d}_{i:06d}_{time_input[i].view(-1).item()}.png"
                            )

                        gt_img = vae.decode(x_latent / 0.18215).sample
                        gt_img = torch.clamp(127.5 * gt_img + 128.0, 0, 255)\
                                .permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
                        for i, sample in enumerate(gt_img):
                            Image.fromarray(sample).save(
                                f"{samples_dir}/{train_steps:07d}_{i:06d}_{time_input[i].view(-1).item()}_gt.png"
                            )
                ...
                if train_steps % args.log_every == 0:
                    ...
                    avg_loss_local = torch.tensor(running_loss / (log_steps * grad_accum), device=device)
                    avg_denoising_local = torch.tensor(running_denoising_loss / (log_steps * grad_accum), device=device)
                    avg_proj_local = torch.tensor(running_proj_loss / (log_steps * grad_accum), device=device)

                    dist.all_reduce(avg_loss_local, op=dist.ReduceOp.SUM)
                    dist.all_reduce(avg_denoising_local, op=dist.ReduceOp.SUM)
                    dist.all_reduce(avg_proj_local, op=dist.ReduceOp.SUM)

                    world = dist.get_world_size()
                    avg_loss = (avg_loss_local.item() / world)
                    avg_denoising_loss = (avg_denoising_local.item() / world)
                    avg_proj_loss = (avg_proj_local.item() / world)

                    if rank == 0:
                        print(f"(step={train_steps:07d}) "
                            f"Train Loss: {avg_loss:.4f}, "
                            f"Train Denoising Loss: {avg_denoising_loss:.4f}, "
                            f"Train Proj Loss: {avg_proj_loss:.4f}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default = "../imagenet/train")
    parser.add_argument("--cutmix_data_path", type=str, default = "../ddd/imagenet/train")
    ...
    parser.add_argument("--embedding-root", type=str, default="/path/to/your/visual_info/intervalXX/")
    parser.add_argument("--embedding-json", type=str, default="/path/to/your/visual_info/intervalXX/total_json.json")
    parser.add_argument("--prompt-root", type=str, default="text/caption_embeddings")
